...
public:
	...
	void addEdge(T u, T v, int dist,bool bidirec = 1){
		...
		if (bidirec) adjlist[v].push_back(make_pair(u, dist));
	}
	...
	void bfs(T u){
		...
		while (!q.empty()){
			...
			for (auto a : (adjlist[front_element])){
				if(!visited[a.first]) q.push(a.first);
			}
		}
	}
	...
	void dfs_helper(T src,map<T,bool> &visited){
		...
		for (auto a : adjlist[src]){
			if (!visited[a.first]) dfs_helper(a.first, visited);
		}
	}	
	...
	void dijkstra(T src){
		...
		set<pair<int, T> > s; s.insert(make_pair(0, src));
		dist[src] = 0;
		while (!s.empty()){
			auto p = *(s.begin());
